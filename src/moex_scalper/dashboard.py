    ...
    const renderStrategy = (parameters, diagnostics, riskControls) => {
      ...
      return renderTable(
        ...
        [
          ...
          ["Ticker Guard Policy", risk?.ticker_guard_policy ?? "—"],
          ["Daily Loss Policy", risk?.daily_loss_policy ?? "—"],
          ...
          ["Adaptive Max Spread", parameters.adaptive_max_spread_bps ?? diagnostics?.adaptive_max_spread_bps ?? "—"],
          ["Adaptive Late Spread", parameters.adaptive_late_session_max_spread_bps ?? diagnostics?.adaptive_late_session_max_spread_bps ?? "—"],
          ["Adaptive Min Impulse", parameters.adaptive_min_impulse_bps ?? diagnostics?.adaptive_min_impulse_bps ?? "—"],
          ["Adaptive Late Impulse", parameters.adaptive_late_session_min_impulse_bps ?? diagnostics?.adaptive_late_session_min_impulse_bps ?? "—"],
          ["Adaptive Min Edge", parameters.adaptive_min_expected_edge_bps ?? diagnostics?.adaptive_min_expected_edge_bps ?? "—"],
          ["Adaptive Late Edge", parameters.adaptive_late_session_min_expected_edge_bps ?? diagnostics?.adaptive_late_session_min_expected_edge_bps ?? "—"],
          ["Post-Cost Edge Floor", parameters.adaptive_expected_edge_after_costs_floor_bps ?? diagnostics?.adaptive_expected_edge_after_costs_floor_bps ?? "—"],
          ["Cost Headroom Floor", parameters.adaptive_cost_headroom_floor_bps ?? diagnostics?.adaptive_cost_headroom_floor_bps ?? "—"],
          ["Impulse/Spread Floor", parameters.adaptive_impulse_spread_ratio_floor ?? diagnostics?.adaptive_impulse_spread_ratio_floor ?? "—"],
          ["Strong Edge Tier", parameters.adaptive_strong_expected_edge_after_costs_bps ?? diagnostics?.adaptive_strong_expected_edge_after_costs_bps ?? "—"],
          ["Strong Ratio Tier", parameters.adaptive_strong_impulse_spread_ratio ?? diagnostics?.adaptive_strong_impulse_spread_ratio ?? "—"],
          ["Workable Time Stop", parameters.adaptive_workable_time_stop_seconds ?? diagnostics?.adaptive_workable_time_stop_seconds ?? "—"],
          ["Regime Filter", diagnostics?.regime_filter_mode ?? "—"],
          ["Strategy Overlay", diagnostics?.strategy_overlay_mode ?? "—"],
          ...
          ["Daily Loss Hit", riskControls?.daily_loss_limit_hit === undefined ? "—" : String(riskControls.daily_loss_limit_hit)],
          ["Daily Loss Enforced", riskControls?.daily_loss_limit_enforced === undefined ? "—" : String(riskControls.daily_loss_limit_enforced)],
          ...
          ["Ticker TimeStop Streak", riskControls?.intraday_ticker_max_consecutive_time_stop_losses ?? "—"],
          ["Paper Guard Cooldown", parameters.paper_ticker_guard_cooldown_seconds ?? riskControls?.paper_ticker_guard_cooldown_seconds ?? "—"],
          ["Session Guarded Tickers", riskControls?.intraday_session_max_guarded_tickers ?? "—"],
          ["Active Ticker Guards", activeGuards.map((item) => `${item.ticker} [${(item.reasons || []).join(", ")}${item.guard_cooldown_until ? ` until ${item.guard_cooldown_until}` : ""}]`).join(" | ") || "—"],
    ...
    const renderTuning = (tuning) => {
      ...
      return renderTable(
        ...
        [
          ...
          ["Overlay Before", tuning.current_strategy_overlay_mode || "—"],
          ["Overlay Candidate", tuning.candidate_strategy_overlay_mode || "—"],
          ["Overlay After", tuning.strategy_overlay_mode_after || "—"],
          ...
          ["Entry Guard Reason", tuning.analysis_edge_floor_guard?.reason || "—"],
          ["Entry Guard Bucket", tuning.analysis_edge_floor_guard?.worst_edge_bucket || "—"],
          ["Entry Guard Next Floor", tuning.analysis_edge_floor_guard?.next_floor_bps || "—"],
    ...
    const renderGovernance = (governance) => {
      ...
      return renderTable(
        ...
        [
          ...
          ["Experiment Status", governance.active_experiment?.status || "—"],
          ["Experiment Trades", governance.active_experiment?.trade_count ?? "—"],
          ["Experiment Net PnL", governance.active_experiment?.summary?.net_pnl_rub ? fmtRub(governance.active_experiment.summary.net_pnl_rub) : "—"],
          ["Experiment Expectancy", governance.active_experiment?.summary?.expectancy_rub ? fmtRub(governance.active_experiment.summary.expectancy_rub) : "—"],
          ["Experiment PF", governance.active_experiment?.summary?.profit_factor ?? "—"],
          ["Experiment Focus", (governance.active_experiment?.focus || []).map((item) => item.message).join(" | ") || "—"],
...
def _default_payload() -> dict[str, object]:
    return {
        ...
        "risk_controls": {
            ...
            "daily_loss_limit_hit": None,
            "daily_loss_limit_enforced": None,
            "paper_continue_after_daily_loss_limit": None,
            "intraday_ticker_loss_limit_rub": None,
            "intraday_ticker_max_consecutive_losses": None,
            "intraday_ticker_max_consecutive_time_stop_losses": None,
            "paper_ticker_guard_cooldown_seconds": None,
